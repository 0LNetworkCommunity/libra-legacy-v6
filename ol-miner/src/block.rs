...
pub mod build_block {
    ...
    pub fn mine_once(config: &OlMinerConfig) -> Result<Block,Error> {
        ...
        // Err(ErrorKind::Io.context(format!("submit_vdf_proof_tx_to_network {:?}", block_dir)).into())
            ...
            return Err(ErrorKind::Io.context(format!("No files found in {:?}", block_dir)).into());
        }
    }
    /// Write block to file
    pub fn mine_and_submit(config: &OlMinerConfig, mnemonic:String, waypoint: Waypoint) ->Result<(),Error> {
            ...
            loop {
                ...
                if waypoint.version() >= 0  {
                    if let Some(ref node) = config.chain_info.node{
                        ...
                        return Err(ErrorKind::Config.context("No Node for submitting transactions").into());
                    }
                } else {
                    return Err(ErrorKind::Config.context("No Waypoint for client provided").into());
                }
