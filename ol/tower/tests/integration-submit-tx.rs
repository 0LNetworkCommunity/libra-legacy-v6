...
pub fn integration_submit_tx() {
    ...
    match fs::remove_dir_all(&swarm_configs_path) {
        Ok(_) => {
            println!("wiping swarm temp directory! {:?}", &swarm_configs_path)
        }
        Err(_) => {}
    }
    ...
    match cmd {
        ...
        Ok(mut swarm_child) => {
            ...
            let tx_params = TxParams::get_tx_params_from_swarm(
                swarm_configs_path.clone(),
                "alice".to_owned(),
                false,
            )
            .unwrap(); // TO write logic
            let config = AppCfg::init_app_configs_swarm(
                swarm_configs_path.clone(),
                swarm_configs_path.join("0"),
                Some(root_source_path.clone().to_path_buf()),
            )
            .unwrap();
            ...
            let (current_block_number, _current_block_path) =
                tower::proof::parse_block_height(&blocks_dir);
            ...
            match check_node_sync(&tx_params, &config) {
                ...
                Err(err) => {
                    ...
                    miner_child.kill().unwrap();
                    std::panic!(
                        "Check node sync failed after block & re-active port: {}",
                        err
                    )
                }
            };
...
fn check_node_sync(tx_params: &TxParams, config: &AppCfg) -> Result<(), Error> {
    ...
    if (current_block_number != remote_height) && (current_block_number - 1) != remote_height {
        bail!(
            "Block heights don't match: Miner: {}, Remote: {}",
