    ...
    match tower_state {
        Ok(response) => match response.into_inner() { 
            Some(s) => { 
                ...
                debug!("epochs_validating_and_mining: {:?}",
                    s.epochs_validating_and_mining);
                debug!("contiguous_epochs_validating_and_mining: {:?}",
                    s.contiguous_epochs_validating_and_mining);
                debug!("epochs_since_last_account_creation: {:?}",
                    s.epochs_since_last_account_creation);
                debug!("actual_count_proofs_in_epoch: {:?}",
