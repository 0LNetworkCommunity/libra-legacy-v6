...
pub fn check(path: PathBuf) {
    let user_data = UserConfigs::get_init_data(&path).expect(&format!("could not parse manifest in {:?}", &path));

    match crate::delay::verify(&user_data.block_zero.preimage, &user_data.block_zero.proof) {
