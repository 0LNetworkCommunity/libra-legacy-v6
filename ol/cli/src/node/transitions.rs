...
use super::{node::Node, states::{MinerEvents, Tower, NodeEvents, OnboardEvents, OnboardState, NodeState}};
...
impl Node {
  ...
  pub fn miner_transition(&mut self, action: MinerEvents, _trigger_action: bool) -> &Self {
    match action {
      MinerEvents::Started => self.vitals.host_state.miner_state = Tower::Mining,
      MinerEvents::Failed => self.vitals.host_state.miner_state = Tower::Stopped,
    };
    self
  }
  ...
  pub fn node_maybe_advance(&mut self, trigger_action: bool) -> &Self {
    match &self.vitals.host_state.node_state {
      ...
      NodeState::FullnodeModeCatchup => {
        if let Ok(sync) = self.check_sync() {
          if sync.is_synced {
