...
impl Runnable for CreateTeamCmd {    
    fn run(&self) {
        ...
        match create_team(&self.team_name, &self.operator_pct_reward, tx_params,  entry_args.save_path) {
...
pub fn create_team(team_name: &str, percent: &u64, tx_params: TxParams, save_path: Option<PathBuf>) -> Result<TransactionView, TxError>{
   maybe_submit(
      transaction_builder::encode_create_team_script_function(team_name.as_bytes().to_vec(), *percent),
      &tx_params,
      save_path,
    )
}