...
impl Runnable for JoinTeamCmd {    
    fn run(&self) {
        ...
        match set_team(team_address, tx_params,  entry_args.save_path) {
            ...
            Err(e) => {
              println!("ERROR: could not submit join team transaction, message: \n{:?}", &e);
              exit(1);
            }
        }
    }
...
pub fn set_team(captain_address: AccountAddress, tx_params: TxParams, save_path: Option<PathBuf>) -> Result<TransactionView, TxError>{
   maybe_submit(
