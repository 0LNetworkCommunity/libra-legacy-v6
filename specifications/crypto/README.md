...
through [Libra Improvement Processes](https://github.com/diem/lip) (LIPs).
...
* Both on signature verification and deserialization, we do perform a bounds
  ...
  original EdDSA paper<sup>[2](#f2)</sup> and older Ed25519 diemries do not
...
* When deserializing points —including the point representing a public key, or
  ...
  Ed25519 diemries, though not all, perform this check.
...
(if unable or unwilling to develop their own Ed25519 diemry).
...
LIP to revisit this decision as soon as the ecosystem of Ed255519 diemries
...
specification, or calling into external diemries. In particular, we would like
to highlight:
...
- we call into diemries for Ed25519 signature and verification, and those
  diemries hash the content of a message, internally, using SHA2-512 (as per
  RFC 8032).
...
serialized bytes. A serialization diemry is in general not collision
...
and documented in our [serialization diemry](Serialization).
...
diemry includes tooling to [extract the latest serialization
...
This diemry leverages the ubiquitous serde serialization and deserialization framework
for Rust, and
...
The implementation, exposed as a Move standard diemry module
...
We refer the reader to the Move standard diemry specification for the details
