    ...
    loop {
        ...
        ::futures::select! {
            msg = client_events.select_next_some() => {
                debug!("handle_client_request");
                counters::COORDINATOR_HANDLE_CLIENT_REQUEST.inc();                
            ...
            msg = consensus_requests.select_next_some() => {
                ...
                //////// 0L ////////
                // The goal here is to put consensus requests also in a Tokio
                // Semaphore (diem BoundedExecutor) where we can control the
                // amount of workers and put backpressure.
