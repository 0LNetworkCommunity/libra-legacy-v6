...
pub static COORDINATOR_HANDLE_CLIENT_REQUEST: Lazy<IntCounter> = Lazy::new(|| {
    register_int_counter!(
        "diem_mempool_coordinator_handle_client_request",
        "Number of times a client request was handled in mempool"
    )
        .unwrap()
});
...
// 0L todo: Related Diem code we patch does not exist anymore. Delete all of these?
