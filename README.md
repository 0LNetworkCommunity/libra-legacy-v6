...
This is an obvious decision. But the task propagated into many subprojects. The Root Account is pervasive in the codebase. So, we had two things to accomplish: (1) replace all its functions with a system 0x0 account, and (2) neuter the Root Account where impossible to remove (e.g., in much of the genesis logic).
...
In the upstream LibraDiem code, accounts are not created permissionlessly.  It starts with a Diem Association Account, which later adds Virtual Asset Service Providers (such as an exchange), which later create user accounts. So all of this had to be replaced (the code remains in our code base for reference, but is deactivated).
...
### Web Monitor
A web interface for a Node's Operator to see reports on the node.
Basic chain info: Epoch number, Waypoint.
Includes Node Health, and account info
Performance reports on all validators in the set.
