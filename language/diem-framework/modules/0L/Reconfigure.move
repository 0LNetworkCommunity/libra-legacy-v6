...
module Reconfigure { // TODO: Rename to Boundary
    ...
    fun process_fullnodes(vm: &signer, nominal_subsidy_per_node: u64) {
        ...
        print(&1800200);
        let miners = Tower::get_miner_list();
        print(&1800201);
        ...
        while (k < Vector::length(&miners)) {
            ...
            if (Tower::node_above_thresh(addr)) {
              let count = Tower::get_count_in_epoch(addr);
              // print(&count);
    ...
    fun propose_new_set(vm: &signer, height_start: u64, height_now: u64): vector<address> {
        ...
        while (i < Vector::length<address>(&top_accounts)) {
            ...
            let mined_last_epoch = Tower::node_above_thresh(addr);
