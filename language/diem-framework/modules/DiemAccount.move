...
//      pragma disable_invariants_in_body;
    ...
    public fun balance<Token>(addr: address): u64 acquires Balance {
        // if (!exists<Balance<Token>>(addr)) { return 0 }; // todo
    ...
    spec schema WritesetPrologueAbortsIf {
        ...
        include PrologueCommonAbortsIf<GAS>{
            transaction_sender,
    ...
    fun writeset_epilogue(
        ...
        epilogue_common<GAS>(dr_account, txn_sequence_number, 0, 0, 0);
...
//      pragma disable_invariants_in_body;
    ...
    spec module {
        ...
        // > TODO: Temporarily weakened due to inv disable problem
        // invariant forall addr: address:
        //     (exists<Balance<GAS>>(addr) || exists<Balance<XDX>>(addr)) <==> Roles::spec_can_hold_balance_addr(addr);
        invariant forall addr: address:
            (exists<Balance<GAS>>(addr) || exists<Balance<XDX>>(addr)) ==> Roles::spec_can_hold_balance_addr(addr);
