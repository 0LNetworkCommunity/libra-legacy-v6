...
address 0x0 {
 ...
 == 0x0, 130102014010);
      ...
      commit_state(&miner_addr, vdf_proof_blob)
    }
    ...
    public fun commit_state(
      ...
      // Get address, note: may not be the miner's address in case of onboarding.
      ...
      // skip this check on test-net, we need ability for tests to send different difficulties.
      ...
      if (! ::exists<MinerProofHistory>(*miner_address)) {
        ...
        Transaction::assert(Transaction::sender() == *miner_address, 130106031010);
        // Verify the blob and update the state.
        verify_and_update_state(*miner_address,vdf_proof_blob, true);
      }
    }
    ...
    // Permissions: private function.
    fun init_miner_state(miner_address: &address){
      // Initialize MinerProofHistory object and give to miner account
      let sign = create_signer(*miner_address);
      move_to<MinerProofHistory>(&sign, MinerProofHistory{
