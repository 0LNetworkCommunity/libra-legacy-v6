...
module Globals {
    ...
    public fun initialize(account: &signer, is_testnet: bool) {
      LibraTimestamp::assert_genesis();
      if (Signer::address_of(account) == CoreAddresses::LIBRA_ROOT_ADDRESS()) {
        move_to<Testnet>(account, Testnet{ is_testnet: is_testnet });
      } 
    }
    ...
    public fun get_epoch_length(): u64 acquires Testnet {
       get_constants().epoch_length
    }

    // Get max validator per epoch
    public fun get_max_validator_per_epoch(): u64 acquires Testnet {
       get_constants().max_validator_per_epoch
    }

    // Get max validator per epoch
    public fun get_subsidy_ceiling_gas(): u64 acquires Testnet {
       get_constants().subsidy_ceiling_gas
    }

    public fun get_epoch_boundary_buffer(): u64 acquires Testnet {
      get_constants().epoch_boundary_buffer
    }

    // Get max validator per epoch
    public fun get_max_node_density(): u64 acquires Testnet {
       get_constants().max_node_density
    }

    // Get the burn accounts
    public fun get_burn_accounts(): vector<address> acquires Testnet {
       *&get_constants().burn_accounts
    }

    public fun get_difficulty(): u64 acquires Testnet {
