...
module Roles {
    ...
    spec module {
        /// The DiemRoot role is only granted in genesis [[A1]][ROLE]. A new `RoleId` with `LIBRA_ROOT_ROLE_ID` is only
        /// published through `grant_diem_root_role` which aborts if it is not invoked in genesis.
        apply ThisRoleIsNotNewlyPublished{this: LIBRA_ROOT_ROLE_ID} to * except grant_diem_root_role, grant_role;
        ...
        /// ParentVASP roles are only granted by DiemRoot [[A6]][ROLE]. A new `RoleId` with `PARENT_VASP_ROLE_ID()` is only
        ...
        /// The DiemRoot role is globally unique [[B1]][ROLE], and is published at LIBRA_ROOT_ADDRESS [[C1]][ROLE].
        ...
        invariant [global, isolated]
            DiemTimestamp::is_operating() ==> spec_has_diem_root_role_addr(CoreAddresses::LIBRA_ROOT_ADDRESS());
