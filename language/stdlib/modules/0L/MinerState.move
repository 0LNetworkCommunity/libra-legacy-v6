  ...
  module MinerState {
    ...
    public fun commit_state(miner: &signer, vdf_proof_blob: VdfProofBlob) acquires MinerProofHistory, ProofsInEpoch {
      ...
      if (!exists<MinerProofHistory>(miner_addr)) {
        ...
        assert(Signer::address_of(miner) == miner_addr, 130106031010);
    ...
    fun verify_and_update_state(
      ...
      assert(Vector::length(&miner_redemption_state.verified_proof_history) > 0, 130107021010);
    ...
    fun check_hash_and_verify(
      ...
      assert(&vdf_proof_blob.challenge == previous_verified_solution_hash, 130108031010);
    ...
    public fun update_metrics(account: &signer, miner_addr: address) acquires ProofsInEpoch, MinerProofHistory {
    ...
    public fun end_redeem_validator_universe(account: &signer)
      ...
      while (i < size) {
          ...
          if ( exists<ProofsInEpoch>( redeemed_addr ) ){
              update_metrics(account, redeemed_addr);
          };
          i = i + 1;
      };
    }
