...
impl<S: Storage> VerifyingClient<S> {
    ...
    pub async fn get_recent_transactions(
        ...
        self.request(MethodRequest::get_recent_transactions(
            start_version,
            limit,
            include_events,
        ))
        .await?
        .and_then(MethodResponse::try_into_get_recent_transactions)
    }
...
impl From<MethodRequest> for VerifyingRequest {
    fn from(request: MethodRequest) -> Self {
        match request {
            ...
            MethodRequest::GetRecentTransactions(start_version, limit, include_events) => {
                verifying_get_recent_transactions(start_version, limit, include_events)
            }
...
fn verifying_get_recent_transactions(
  start_version: Version,
  limit: u64,
  include_events: bool,
) -> VerifyingRequest {
  let request = MethodRequest::GetRecentTransactions(start_version, limit, include_events);
  let subrequests = vec![MethodRequest::GetTransactionsWithProofs(
      start_version,
      limit,
      include_events,
  ...
  let callback: RequestCallback = |ctxt, subresponses| {
      ...
      let txs_with_proofs_view = if let Some(txs_with_proofs_view) = maybe_txs_with_proofs_view {
          ...
          return Ok(MethodResponse::GetRecentTransactions(Vec::new()));
      };
      ...
      Ok(MethodResponse::GetRecentTransactions(txn_list_view.0))
  };
  VerifyingRequest::new(request, subrequests, callback)
}
