//# init --parent-vasps Alice Bob X Carol Y Dave
// Alice X Y:          validators with 10M GAS
// Bob Carol Dave: non-validators with  1M GAS

// Create three end user miner accounts Bob Carol Dave
// Dave will not mine above threshold

// Bob, Carol, Dave are end-users running the Carpe app, and submitting miner proofs.
// He is the only one in the epoch submitting proofs. He should get
// the entirety of the Identity Subsidy pool avaialable (one validator's worth)
...
// so we need Alice to be a Case 1 validator so that there is a subsidy to
// be paid to validator set.
...
script {  
    ...
    fun main() {
        ...
        // bob and carol EXPECT TO SHARE half the identity subsidy but DONT GET IT
        assert!(DiemAccount::balance<GAS>(@Bob) != ending_balance, 735711);
        assert!(DiemAccount::balance<GAS>(@Bob) == starting_balance, 735712);

        assert!(DiemAccount::balance<GAS>(@Carol) != ending_balance, 735713);
        assert!(DiemAccount::balance<GAS>(@Carol) == starting_balance, 735714);
        // dave's balance is unchanged
        assert!(DiemAccount::balance<GAS>(@Dave) == starting_balance, 735715);
