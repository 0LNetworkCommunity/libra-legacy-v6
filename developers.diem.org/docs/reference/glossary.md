...
- The **Diem Blockchain** is a ledger of immutable transactions agreed upon by the validator nodes on the Diem network (the network of validator nodes).

### Diem Core

- Diem Core is the open-source implementation of the Diem protocol published by the Diem Association.
- This software is the first implementation of the Diem protocol and the Move language.
- Diem Core includes both node and client functionalities.
...
- A **validator** is an entity of the Diem ecosystem that validates the Diem Blockchain. It receives requests from clients and runs consensus, execution, and storage.
