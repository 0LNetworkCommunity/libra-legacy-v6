...
### Run a Local Diem Network and Spawn a CLI Client
To start a local Diem network and spawn a CLI client that connects to this local network, run:
```bash
...
* `-p | -—port` &mdash; The public port of the destination Diem Blockchain for the client to connect with, the default value is 30307.
* `-n | --mnemonic_file` &mdash; The file location from which to load the mnemonic word for user account address/key generation. If not passed, a new mnemonic file is generated in the current directory by diem_wallet.
* `-s | --validator_set_file` &mdash; File location from which to load config of trusted validators. It is used to verify validator signatures in validator query response. The file should at least include public key of all validators trusted by the client &mdash; which should typically be all validators on the network. To connect to testnet, use 'diem/scripts/cli/trusted_peers.config.toml'. Can be generated by diem-config for local testing: `cargo run --bin diem-config` but the preferred method is to simply use diem-swarm to run local networks.
