...
//////// 0L ////////
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
pub struct TowerStateResourceView {
...
impl TryFrom<AccountState> for TowerStateResourceView {
    ...
    fn try_from(state: AccountState) -> Result<TowerStateResourceView, Error> {
        let m = state.get_miner_state()?.unwrap_or(bail!("could not get tower state"));
...
impl TryFrom<AccountState> for OracleUpgradeStateView {
    ...
    fn try_from(state: AccountState) -> Result<OracleUpgradeStateView, Self::Error> {
        let m = state.get_oracle_state()?.unwrap_or(bail!("could not get upgrade oracle state"));
