...
use crate::{
    ...
    views::{
        ...
        MetadataView, TowerResourceView, OracleUpgradeStateView, StateProofView,
...
use diem_json_rpc_types::request::{
    ...
    GetMetadataParams, GetTowerParams, GetNetworkStatusParams, GetStateProofParams,
...
impl<'a> Handler<'a> {
    ...
    pub async fn handle(&self, method_request: MethodRequest) -> Result<Value, JsonRpcError> {
        let response: Value = match method_request {
            ...
            //////// 0L ////////
            MethodRequest::GetTowerView(params) => {
