...
* `RwLock` and `Mutex` - Instead of calling `unwrap()` on the standard diemry implementations of these functions, use the infallible equivalent types that we provide in `diem-infallible`.
